...
object Day2 {
  ...
  case class pwordPolicy(lowest: Int, highest: Int, letter: Char)
  // Used for the xor function later on. Taken from https://docs.scala-lang.org/tour/operators.html
  ...
  // Regex that will be used to extract the necessary elements of each String in the List above.
  // The elemtns of it are: some number of 0-9 digits, some number of 0-9 digits, a letter a-z, a colon, any number of letters a-z
  val polReg = """(\d+)-(\d+) (\w): (\w+)""".r

  // Working on using Traits more in Scala. Might not be the best implementation of them
  sealed trait Valid {
    def isValid(pwordPolicy: pwordPolicy, pword: String): Boolean
  }

  object Part1 extends Valid {
    // https://alvinalexander.com/scala/scala-count-number-occurrences-character-in-string/
    // What this should do:
    // 1) Take a pwordPolicy
    // 2) Get the count of the char from the pword
    // 3) See if the count is between the min and max inclusive
    // 4) Return true or false
    override def isValid(pwordPolicy: pwordPolicy, pword: String): Boolean = {
      val count = pword.count(_ == pwordPolicy.letter) 
      count match {
        case count if count >= pwordPolicy.lowest && count <= pwordPolicy.highest => {
          true
        }
        case _ => {
          false
    ...
    // Parsing Strings with Regex: https://www.tutorialspoint.com/scala/scala_regular_expressions.htm
    //                             https://stackoverflow.com/questions/25510899/how-do-i-use-scala-regular-expressions-to-parse-a-line-of-text
    // Working with Tuples: https://www.tutorialspoint.com/scala/scala_tuples.htm
    def convertToPolicyAndPword(rawInput: String): (pwordPolicy, String) = rawInput match {
      case polReg(lowest, highest, letter, password) => {
        // A point of note: We can use the .head function to take the letter that is given and turn it into a char
        (pwordPolicy(lowest.toInt, highest.toInt, letter.head), password)
      }
    }

    // Could make this more funcitonal by making the two functions that are used parameters for the createValidList function
    def createValidList(entries: List[String]): List[Boolean] = {
      var validOrNot = ArrayBuffer[Boolean]()
      
      for (entry <- entries) {
        val policyAndPword = convertToPolicyAndPword(entry)
        val validity = isValid(policyAndPword._1, policyAndPword._2)
        validOrNot += validity
      }
  ...
  object Part2 extends Valid {
    override def isValid(pwordPolicy: pwordPolicy, pword: String): Boolean = {
      pword match {
        case pword if xor(pword(pwordPolicy.lowest-1) == pwordPolicy.letter, pword(pwordPolicy.highest - 1) == pwordPolicy.letter) => {
          true
        }
        case _ => {
          false
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(s"Day 2, Part 1: ${Part1.createValidList(entries).count(_ == true)}")
    println(s"Day 2, Part 2: ${Part2.createValidList(entries).count(_ == true)}")
