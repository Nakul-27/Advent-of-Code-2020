// Did some reading and I think that we're going to use maps for this problem.
// Maps: https://docs.scala-lang.org/overviews/collections/maps.html
// Maps can also be used nicely with sets, so we're going to take that appraoch as well.
...
byr:1959
  ...
  // We can think of each field of the passport as having a key and a value.
  // This is where the key is a field and the value of that field
  type PassportFields = Map[String, String]

  // The question tells us that there are fields required for the passport to be valid,
  // so we'll use a Set to house those since Sets have a constant lookup time.
  // Essentially the only field that isn't required is the "cid" field.
  val necessaryFields = Set("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid")

  // More work with sealed traits to try to get some practice with them.
  sealed trait Over {
    def validPassport(passportFields: PassportFields): Boolean

    // This will be the same for both parts since after Part 1,
    // It is revealed that the only difference between the parts 
    // is how we classify a passport as valid. 
    // This function should stay the same.
    def numValidPassports(passports: List[PassportFields]): Int = {
      // A simple for loop to count the number of valid passports. 
      // TODO: Look for a funciton that can help do this more elegantly
      var count = 0
  ...
  object Part1 extends Over {
    // In this we need to check if each passportFields contains all of the necessary fields.
    // From what I learned in Discrete Math and Theory of Computation at Norwich,
    // we can say that the necessary fields need to be a subset of the keys in passportFields.
    // This is because passortFields can contain a field that isn't in the necessary fields,
    // however all of the necessary fields must be in passport fields.
    // After a quick Google Search, I found that there's a nice subsetOf function for sets: https://www.geeksforgeeks.org/scala-set-subsetof-method-with-example/
    // Or this article: https://www.tutorialspoint.com/scala/scala_sets.htm
    // We can loop through all of the values in each PassportField, however after looking up how to get all of the keys in a Map,
    // this article came up: https://www.geeksforgeeks.org/scala-set-subsetof-method-with-example/, which shows the .keySet function.
    override def validPassport(passportFields: PassportFields): Boolean = {
      necessaryFields.subsetOf(passportFields.keySet)
    }
  }

  // Like all of the other problems, we need to parse the inputs.
  // We're going to do this in two stages.
  // 1) We can parse the input and separate the Passports as a whole.
  // 2) Separate the passports into their fields, which would be a List of PassportFields
  // 3) Parse the individual fields and get the key and value for each field of each Passport. We can do this with a regex.
  // 4) Feed the results of step 2 back into step 1 in order to create List of PassportsFields.
  // We need to be able to split each passport in to the individual words.
  // After some research, I found this: https://teamtreehouse.com/community/what-is-regular-expression-s
  // Which shows that we can split by the regex \\s+ or something like that. We'll give that a shot.

  // We can think of the regex as any number of characters, a colon, and any number of characters.
  // This works since we'll have separated any white space already.
  val regexForFields = """(.*):(.*)""".r

  // Step 1 and Step 4
  def createList(input: String): List[PassportFields] = {
    // We take the whole input, split it by the blank line between each entry.
    // From here we convert those into a List and put each input through the createPassportFields function.
    input.split("\n\n").toList.map(createPassportFields)
  }
  
  // Step 2
  def createPassportFields(indivPassport: String): PassportFields = {
    // We take the individual passport field, and then split it by the whitespace, and then pass it through the parseFieldRegex function.
    // From here we convert the answer that we get to a Map.
    indivPassport.split("\\s+").map(parseFieldRegex).toMap
  }

  // Step 3
  // We'll return the Field and the Unique Id that we'll then turn into a Map.
  def parseFieldRegex(indivPassportField: String): (String, String) = {
    indivPassportField match {
      case regexForFields(field, uid) => (field, uid)
    }
  }

  // This part proves to be considerably more difficult than the last, in my opinion. 
  // This is simply because there are more conditions to check. 
  // I think that the best way to approach all of the condition checking is mainly with regexes and to see if things are within a range.
  object Part2 extends Over {
    // Necessary Regexes
    // A number followed by either "in" or "cm".
    // We'll have to check the range and whether the second part is either "in" or "cm."
    val heightRegex = """(\d+)(.+)""".r
    // A "#" followed by 6 digits from 0-9 or a-f
    val hairColor = """#[a-f0-9]{6}""".r
    // 9 digits of 0-9 including leading 0s
    val passportId = """\d{9}""".r
    // For the Eye color, we can just use a set to see if they have a good eye color.
    val eyeColor = Set("amb", "blu", "brn", "gry", "grn", "hzl", "oth")

    // In the spirit of being more functional as well as looking at what some talks that I've watched have shown,
    // I'm going to write my own "isBetween" function.
    def isBetween(start: Int, end: Int, number: Int): Boolean = {
      // We can use a cool built in scala to check the range
      // Article: https://alvinalexander.com/scala/how-to-use-range-class-in-scala-cookbook/
      (start to end).contains(number)
    }

    // Checking to see if the height is in range.
    def checkHeight(height: String): Boolean = {
      height match {
        case heightRegex(num, unit) => {
          if (unit == "cm") isBetween(150, 193, num.toInt)
          else  isBetween(59, 76, num.toInt)
        }
      }
    }

    // I would like to sort the values of the passport Fields by key so that way we get the same every time.
    // Article: https://www.geeksforgeeks.org/how-to-sort-a-scala-map-by-key/
    // We should get the following order: byr, cid, ecl, eyr, hcl, hgt, iyr, pid
    override def validPassport(passportFields: PassportFields): Boolean = {
      var validData = passportFields.forall { case (key, data) => checkData(key, data) }
      (necessaryFields.subsetOf(passportFields.keySet) && validData)
    }

    def checkData(key: String, data: String): Boolean = {
      var validFields = true
      key match {
        case "byr" => {
          if (!isBetween(1920, 2002, data.toInt)) validFields = false
        }
        case "cid" => {
          None
        } 
        case "ecl" => {
          if (!eyeColor.contains(data)) validFields = false
        }
        case "eyr" => {
          if (!isBetween(2020, 2030, data.toInt)) validFields = false
        }
        case "hcl" => {
          if (!(hairColor matches data)) validFields = false
        }
        case "hgt" => {
          if (!checkHeight(data)) validFields = false
          
        }
        case "iyr" => {
          if (!isBetween(2010, 2020, data.toInt)) validFields = false
        }
        case "pid" => {
          if (!(passportId matches data)) validFields = false
        }
      }
      validFields
    } 
  ...
  def main(args: Array[String]): Unit = {
    println(s"Day 4, Part 1: ${Part1.numValidPassports(createList(input))}")
    println(s"Day 4, Part 2: ${Part2.numValidPassports(createList(input))}")
  }
}
